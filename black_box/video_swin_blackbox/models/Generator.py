...
class Generator(nn.Module):
    ...
    def __init__(self, in_dim=120, latent_dim=3, n_class=400, ch=16, n_frames=8, hierar_flag=False, frame_repeat=4, size_to=224):
        super().__init__()
    ...
    def forward(self, x, class_id):
        ...
        # print(noise_emb.shape, class_emb.shape)
        ...
        y = size_changer(y, self.n_frames * self.frame_repeat, self.size_to)
        y_pre = y_pre.view(-1, C, self.n_frames, W, H)
        y_pre = torch.repeat_interleave(y_pre, self.frame_repeat, dim=2)
        y_pre = size_changer(y_pre, self.n_frames * self.frame_repeat, self.size_to)
        # print(y.shape)
        return (y, y_pre)

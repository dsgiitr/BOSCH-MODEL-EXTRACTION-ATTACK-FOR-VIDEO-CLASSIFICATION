...
train_kinetics = datasets.Kinetics(TRAIN, frames_per_clip= FRAME, split='train', num_classes= '600', step_between_clips= 16, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
...
def train_with_extraction(model, victim):
    ...
    sc1 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optim1, T_0 = 200)
    sc2 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optim2, T_0 = 200)
    ...
    for idx in range(10):
      ...
      for step,(video, label) in enumerate(train_dl):
          ...
          if (step % 100):
            torch.save(model, 'swin_to_r21_weights_cosine.pth')
      ...
      with torch.no_grad():
          ...
          # print(f'Accuracy : {get_accuracy(torch.argmax(prediction, dim=1).tolist(), label)}')
              ...
              #acc1.append(topk(prediction.cpu(), torch.argmax(l_, dim=1).cpu(), 1))
              #acc5.append(topk(prediction.cpu(), torch.argmax(l_, dim=1).cpu(), 5))

              list_topk_accs = accuracy(output=prediction.cpu(), target=torch.argmax(l_, dim=-1, keepdim=False).cpu(), topk=(1, 5))
              acc1.append(list_topk_accs[0])
              acc5.append(list_topk_accs[1])

              #print(acc1, acc5)
              print(torch.mean(torch.tensor(acc1)), torch.mean(torch.tensor(acc5)))

...
train_kinetics = datasets.Kinetics(TRAIN, frames_per_clip= FRAME, split='train', num_classes= '400', step_between_clips=32, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
test_kinetics = datasets.Kinetics(VALIDATION, frames_per_clip= FRAME, split='val', num_classes= '400', step_between_clips= 200000000, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
...
# train_ucf = datasets.Kinetics(DUMMY, frames_per_clip= FRAME, split='val', num_classes= '400', step_between_clips= 16, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
...
# train_hmdb51 = datasets.Kinetics(DUMMY, frames_per_clip= FRAME, split='val', num_classes= '400', step_between_clips= 16, transform = train_transform,  download= False, num_download_workers= 1, num_workers= 80)
train_ds1 = torch.utils.data.ConcatDataset([train_ucf, train_hmdb51])
train_ds2 = torch.utils.data.ConcatDataset([train_kinetics])
test_ds = test_kinetics
train_dl1 = DataLoader(train_ds1, collate_fn=collate_fn, batch_size = batch_size, shuffle = True);
train_dl2 = DataLoader(train_ds2, collate_fn=collate_fn, batch_size = batch_size, shuffle = True);
...
def train_with_extraction(model, victim):
    ...
    for idx in range(10):
      ...
      rloss = 0.0;
      model.train()
      for step,(video, label) in enumerate(train_dl1):
          #if step>-1:
          #   break
          torch.cuda.empty_cache()
          optim1.zero_grad()
      ...
      for step,(video, label) in enumerate(train_dl2):
         # if step>-1:
          #    break
          torch.cuda.empty_cache()
          optim1.zero_grad()
          ...
          video = size_changer(video, FRAME, 112)
          pred = model(video)
          true_loss = torch.nn.functional.cross_entropy(pred,label)
          pred = torch.nn.functional.log_softmax(pred)
          loss = criterion(pred,label_)
          true_wt = 10
          loss += true_loss*true_wt
          loss/=(1+true_wt)
          rloss+=loss.item()
          loss.backward()
          optim1.step()
          ...
          if (step % 100):
            torch.save(model, 'swin_to_r21_kd.pth')

      print(f'avg loss: {rloss/(len(train_dl1)+len(train_dl2))}')
      print('evaluation:')
      model.eval()
      with torch.no_grad():
          acc1 = []
